...
config = {
    ...
    'transfer': {
        'dnn_model': 'dnn_model/checkpoint_NanoString.t7',
        'gpu': '0',
        'batch_size': 4096,
    },
    'train': {
...
def spatial_classification_tool(config, data_name):
    ...
    # Compute accuracy (only for NanoString).
    if dataset == 'NanoString':
        indices = np.where(adata.obs['celltype_refined'].isin(label_names))[0]
        adjusted_pr = adata.obs['pseudo_class'][indices].to_numpy()
        adjusted_gt = adata.obs['celltype_refined'][indices].to_numpy()
...
if __name__ == '__main__':
    data_list = [f'nanostring_fov{i+1}_sampledata' for i in range(20)]
