...
config = {
    'data': {
        'data_dir': 'dataset/Slide_seq/',
        'save_dir': 'result/Slide_seq/',
        'dataset': 'Slide_seq',
        'radius_ratio': 1.0,
    },
    'preprocess': {
    ...
    'transfer': {
        'dnn_model': 'dnn_model/checkpoint_Slide-seq_DM1.t7',
        'gpu': '0',
        'batch_size': 4096,
    },
    'train': {
...
def spatial_classification_tool(config, data_name):
    ...
    # Sample center area for time test.
    if config['data']['radius_ratio'] < 1:
        adata.obsm['spatial'] = adata.obsm['spatial'] - adata.obsm['spatial'].mean(0)
        radius = adata.obsm['spatial'].max(0).mean() * config['data']['radius_ratio']
        adata = adata[(adata.obsm['spatial']**2).sum(1) < radius**2].copy()
    ...
    if dataset == 'Slide_seq':
        adjusted_pr = adata.obs['pseudo_class'].to_numpy()
        adjusted_gt = adata.obs['ct_name'].replace(
            ['ES', 'Differentiating SPG', 'RS', 'SPC'], ['Elongating', 'SPG', 'STids', 'Scytes']).to_numpy()
        acc = (adjusted_pr == adjusted_gt).sum() / len(adjusted_gt) * 100.0
    ...
    sc.pp.log1p(adata)
    adata_X = adata.X - adata.X.mean(0)
    adata_X = adata_X / (adata.X.std(0) + 1e-10)

    # Construct spatial graph.
    gene_mat = torch.Tensor(adata_X)
    if dataset in ('Stereoseq', 'Slide_seq'):  # PCA
    ...
    if dataset == 'Slide_seq':
        adjusted_pr = celltype_pred.to_numpy()
        adjusted_gt = adata.obs['ct_name'].replace(
            ['ES', 'Differentiating SPG', 'RS', 'SPC'], ['Elongating', 'SPG', 'STids', 'Scytes']).to_numpy()
        acc = (adjusted_pr == adjusted_gt).sum() / len(adjusted_gt) * 100.0
    ...
    # Compute time cost.
    time4 = time.time()
    print(' S1 infer time: %.2fs' % (time2 - time1))
    print(' S2 train time: %.2fs' % (time3 - time2))
    print(' S2 infer time: %.2fs' % (time4 - time3))
...
if __name__ == '__main__':
    data_list = ['DM1', 'DM2', 'DM3', 'WT1', 'WT2', 'WT3']
